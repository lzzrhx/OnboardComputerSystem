...
#Get weather log
sqlite_file='/home/laserwolf/db/weather.db'
...
while True:
  ...
  weather_temp1=float(row[2])
  weather_temp2=float(row[3])
  weather_temp3=float(row[4])
  weather_winddir=float(row[5])
  weather_windspd=float(row[6])
  #Format Entry number
  weather_numformatfull='%05d' %(weather_num)
  #Format time
  weather_timeformatfull=datetime.fromtimestamp(float(weather_time)).strftime('%d.%m.%Y %H:%M')
  #Format baro
  weather_baroformat='{0:.1f}'.format(float(weather_baro/100))
...
#Get stats
sqlite_file='/home/laserwolf/db/ocs.db'
conn=sqlite3.connect(sqlite_file)
c = conn.cursor()
